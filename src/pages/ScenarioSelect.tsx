...
import type { Scenario, TrainingRecord, InProgressState } from '@/data/types'
        ...
        {records.slice(0, 5).map((record) => (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-gray-400">{formatDate(record.completedAt)} · {record.totalEvents}题</p>
            </div>
...
export default function ScenarioSelect() {
  ...
  const { selectScenario, trainingRecords, inProgressState, initFromStorage, loadTrainingRecord } = useStore()
  ...
  const handleStart = (scenarioId: string, resume: boolean) => {
    ...
    setTimeout(() => {
      selectScenario(scenarioId, resume)
      ...
      <div className="relative mx-auto max-w-4xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        <div className="rounded-xl border border-dashed border-gray-300 bg-white/60 px-6 py-4 text-center text-sm text-gray-400 backdrop-blur-sm">
          <p>
            ...
            选择场景 → 查看现场概况 → 逐题判断旁站要点 → 填写判定依据 → 获取复盘报告
          </p>
        </div>
      </div>
